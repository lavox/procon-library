...
// https://github.com/lavox/procon-library/blob/main/lib/primitive/IntIntMap.java
public class IntIntMap {
	...
	public IntIntMap(int initialCapacity, int defaultValue, float loadFactor) {
	...
	private void _insert(int idx, int key, int value) {
		keys[idx] = key;
		values[idx] = value;
		size++;
	}
	...
	public void replaceAll(IntBinaryOperator function) {
		if (keys[capacity] != EMPTY_FOR_EXTRA) values[capacity] = function.applyAsInt(keys[capacity], values[capacity]);
		for (int i = 0; i < capacity; i++) {
			if (keys[i] != EMPTY) {
