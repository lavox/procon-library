...
// https://github.com/lavox/procon-library/blob/main/lib/primitive/LongObjMap.java
public class LongObjMap<V> {
	...
	public LongObjMap(LongObjMap<V> from) {
	...
	private void _insert(int idx, long key, V value) {
		keys[idx] = key;
		values[idx] = value;
		size++;
	}
	...
	@SuppressWarnings("unchecked")
	public V computeIfPresent(long key, RemappingFunction<? super V, ? extends V> remappingFunction) {
		int idx = index(key);
		if (idx >= 0) {
	...
	@SuppressWarnings("unchecked")
	public V compute(long key, RemappingFunction<? super V, ? extends V> remappingFunction) {
		ensureCapacity();
		int idx = index(key);
		if (idx >= 0) {
			V v = remappingFunction.apply(key, (V)values[idx]);
	...
	@SuppressWarnings("unchecked")
	public V merge(long key, V value, MergeFunction<? super V, ? extends V> remappingFunction) {
	...
	@Override
	public LongObjMap<V> clone() {
		return new LongObjMap<>(this);
	}

	@FunctionalInterface
	public interface RemappingFunction<V1, V2> {
		public V2 apply(long key, V1 value);
	}
	@FunctionalInterface
