...
public class Main {
	...
	public void solve() {
		...
		int N = sc.nextInt();
		int M = sc.nextInt();
		long[][] dist = WarshallFloyd.createInitTable(N + 1);
		for (int i = 0; i < M; i++) {
			int A = sc.nextInt() - 1;
			int B = sc.nextInt() - 1;
			long C = sc.nextLong();
		...
		for (int q = 0; q < Q; q++) {
			...
			if (k == 1) {
				...
				int y = sc.nextInt() - 1;
				long t = sc.nextLong();
				dist = WarshallFloyd.updateEdge(dist, x, y, t);
				dist = WarshallFloyd.updateEdge(dist, y, x, t);
			} else if (k == 2) {
				int x = sc.nextInt() - 1;
				dist = WarshallFloyd.updateEdge(dist, x, N, T);
				dist = WarshallFloyd.updateEdge(dist, N, x, 0);
			} else {
				long sum = 0;
				for (int i = 0; i < N; i++) {
					for (int j = i + 1; j < N; j++) {
						if (dist[i][j] == WarshallFloyd.INF) continue;
						sum += dist[i][j] * 2;
					}
				}
				ans.add(sum);
