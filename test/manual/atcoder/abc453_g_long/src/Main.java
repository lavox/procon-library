...
public class Main {
	...
	public void solve() {
		...
		int N = sc.nextInt();
		int M = sc.nextInt();
		LongPersistentSegmentTree[] A = new LongPersistentSegmentTree[N];
		A[0] = new LongPersistentSegmentTree(M, (a, b) -> a + b, 0);
		for (int i = 1; i < N; i++) {
			A[i] = A[0];
		}
		int Q = sc.nextInt();
...
// === begin: data_structure/segment_tree/LongPersistentSegmentTree.java ===
class LongPersistentSegmentTree {
	private final int n;
...
// === end: data_structure/segment_tree/LongPersistentSegmentTree.java ===
