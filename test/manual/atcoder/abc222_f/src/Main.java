...
public class Main {
	...
	public void solve() {
		...
		LongRerooting tree = new LongRerooting(N) {
			@Override
			...
			@Override
			public long mergeSubtree(long x, Edge e) {
				return C[e.id() / 2] + Math.max(x, D[e.to()]);
			}
			@Override
			public long nodeValue(long x, int v) {
				return x;
			}
		};
		for (int i = 0; i < N - 1; i++) {
...
// === begin: graph/LongRerooting.java ===
