...
public class Main {
	...
	public void solve() {
		...
		for (int i = 0; i < N; i++) {
			...
			String S = sc.next();
			for (int k = 0; k < K; k++) {
				bs[i].set(k, T.charAt(k) == S.charAt(k));
			}
		}
		
		int Q = sc.nextInt();
		Trie trie = new DenseTrie(2, (N + Q) * K);
		TrieMultiset tset = new TrieMultiset(trie, (N + Q) * K);
		for (int i = 0; i < N; i++) {
			int _i = i;
			tset.add(K, l -> bs[_i].get(l) ? 0 : 1);
		}
		boolean[] ans = new boolean[Q];
		for (int q = 0; q < Q; q++) {
			int i = sc.nextInt() - 1;
			int j = sc.nextInt() - 1;
			tset.remove(K, l -> bs[i].get(l) ? 0 : 1);
			bs[i].set(j, !bs[i].get(j));
			tset.add(K, l -> bs[i].get(l) ? 0 : 1);
			ans[q] = check(bs[i], trie, tset, K, M);
		}
		printYesNo(ans, LF);
	}
	boolean check(BitSet bs, Trie trie, TrieMultiset tset, int K, int M) {
		int node = Trie.ROOT;
		int rest = M;
		for (int j = 0; j < K; j++) {
			if (bs.get(j)) {
				node = trie.getChild(node, 0);
				if (tset.passCount(node) <= rest) return true;
			} else {
				int cnt = tset.passCount(trie.getChild(node, 0));
				if (cnt >= rest) return false;
				rest -= cnt;
				node = trie.getChild(node, 1);
			}
		}
		return false;
	}
