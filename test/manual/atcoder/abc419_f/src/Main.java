...
public class Main {
	...
	public void solve() {
		...
		ac.buildLink();
		int[] out = new int[ac.nodeSize()];
		for (int ni = 0; ni < ac.nodeSize(); ni++) {
			AhoCorasick.Node n = ac.node(ni);
			ArrayList<AhoCorasick.Node> oarr = n.outputWords();
			if (oarr == null) continue;
			for (AhoCorasick.Node o: oarr) {
				out[ni] |= 1 << o.wordId();
			}
		}
		...
		int N2 = 1 << N;
		int W = ac.nodeSize();
		long[][] dp = new long[N2][W];
		dp[0][0] = 1;
		for (int i = 0; i < L; i++) {
			long[][] ndp = new long[N2][W];
			for (int wi = 0; wi < W; wi++) {
				AhoCorasick.Node n = ac.node(wi);
				for (int b = 0; b < N2; b++) {
					if (dp[b][wi] == 0) continue;
					for (int c = 0; c < K; c++) {
						AhoCorasick.Node nx = n.next(c);
						int nb = b | out[nx.id()];
						ndp[nb][nx.id()] = (ndp[nb][nx.id()] + dp[b][wi]) % MOD;
					}
				}
			}
			dp = ndp;
