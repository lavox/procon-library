...
public class Main {
	...
	public void solve() {
		...
		for (int i = 0; i < M; i++) {
			...
			int B = sc.nextInt() - 1;
			int W = sc.nextInt();
			g.addDirEdge(A, B, i, W);
		}
		g.build();
		XorGraph xg = new XorGraph(g, 1 << 10);
		ShortestPath.Dist dist = Bfs01.search(xg, xg.v(0, 0));
		int ans = -1;
		for (int x = 0; x < xg.layerSize(); x++) {
			if (dist.dist[xg.v(N - 1, x)] != ShortestPath.INF) {
				ans = x;
				break;
			}
		}
	...
	class XorGraph extends LayeredGraph {
		XorGraph(Graph base, int numLayer) {
			super(base, numLayer);
		}
		@Override
		protected void forEachBaseEdge(int bv0, int layer0, int bv1, int eid, long bcost, EdgeConsumer action) {
			acceptAction(bv0, layer0, bv1, layer0 ^ (int)bcost, eid, 1, action);
		}
	}
...
// === begin: graph/LayeredGraph.java ===
abstract class LayeredGraph implements Graph {
