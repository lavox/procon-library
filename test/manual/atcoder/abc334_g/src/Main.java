...
public class Main {
	...
	public void solve() {
		...
		SimpleGraph g = new SimpleGraph(H * W);
		int green = 0;
		for (int i = 0; i < H; i++) {
			for (int j = 0; j < W; j++) {
				...
				if (i < H - 1 && S[i + 1].charAt(j) == '#') {
					g.addUndirEdge(i * W + j, (i + 1) * W + j);
				}
				if (j < W - 1 && S[i].charAt(j + 1) == '#') {
					g.addUndirEdge(i * W + j, i * W + j + 1);
				}
			}
		}
		g.build();
		...
		ModInt ans = mop.zero();
		ModInt cc = mop.create(ll.componentCnt() - (H * W - green));
		int cnt = 0;
		for (int i = 0; i < H; i++) {
			for (int j = 0; j < W; j++) {
				if (S[i].charAt(j) == '.') continue;
				LowLink.Node n = ll.node(i * W + j);
				if (g.edgeSize(n.id()) == 0) {
